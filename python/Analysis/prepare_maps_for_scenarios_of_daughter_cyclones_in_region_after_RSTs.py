    ...
    if slide_counter > 132:
        if row['Omit'] != 'X':
            ...
            while time_offset < len(curr_track):
                ...
                geop_map = np.squeeze(geop_data[index_day, 0, :, :])
                ...
                fig.suptitle(req_date + ", offset: " + str(time_offset*6) + ' Hrs.\nTrack Length: ' + track_length_hours +
                             ', Start Radius: ' + str(start_gradient) + ', Max Radius: ' + str(max_gradient) +
                             ', Max Radius in 48 Hrs: ' + str(max_gradient_48), fontsize=20)
                ...
                if save_maps:
                    ...
                    filename = directory + str(slide_counter) + "_" + map_name + "_map_number_" + str(floor(time_offset/2)+1) + ".png"
